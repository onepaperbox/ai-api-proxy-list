...
1. Extract the domain from current lines
2. Look up the original name from the map
3. Reconstruct with proper markdown link and all columns
...
for line in lines[2:]:
    ...
    # The current format (broken) is: | N | domain_or_name | models | latency |
    # We need to extract the domain and latency
    parts = stripped.split(" | ")
    
    current_num = parts[0].lstrip("| ").strip()
    
    # Extract domain: find it somewhere in the line, or use the link text
    # Look for a markdown link pattern anywhere
    link_match = re.search(r'\[([^\]]+)\]\(https?://([^\)]+)\)', stripped)
    
    if link_match:
        # Link still exists! Use it directly
        domain = link_match.group(2).split('/')[0].replace('www.', '')
        link_text = link_match.group(1)
        full_link = link_match.group(0)
        
        # Find models and latency
        # Looking at format: parts that aren't number or link
        name = name_map.get(domain, link_text)
        
        # Get models from parts (after link)
        link_idx = -1
        ...
        if link_idx >= 0 and link_idx + 2 < len(parts):
            models = parts[link_idx + 1].strip()
            latency = parts[link_idx + 2].strip().rstrip(" |").strip()
            new_line = f"| {num} | {name} | {full_link} | {models} | {latency} |"
            new_lines.append(new_line)
        else:
            new_lines.append(line)
    else:
        # Link column is gone - reconstruct from what remains
        # parts: ['| N', 'domain_or_name', 'models', 'latency |']
        domain_or_name = parts[1].strip()
        models = parts[2].strip() if len(parts) > 2 else ""
        latency = parts[3].strip().rstrip(" |") if len(parts) > 3 else ""
        
        # Determine if domain_or_name is a domain or the original name
        if '.' in domain_or_name and not domain_or_name.startswith('http'):
            # It's a domain, look up the name
            name = name_map.get(domain_or_name, domain_or_name)
            domain = domain_or_name
        else:
            name = domain_or_name
            domain = domain_or_name
        
        # Check if it has http scheme
        ...
        try:
            full_link = f"[{domain_clean}](https://{domain_clean})" if not domain_clean.startswith('http') else f"[{domain_clean}]({domain_clean})"
        except:
            full_link = domain_clean
...
rows = [r for r in verify[verify_start:].split("\n") if r.strip().startswith("|") and "名称" not in r and ":---" not in r]
